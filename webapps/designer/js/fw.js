...
var EventName = {
	...
	dialogVisible: function(dialogId) {
  	  return VAR_EVENT_ROOT+".dialog."+dialogId+".visible";
    }
...
EventQueue.prototype.subscribe = function(eventName, subscriber) {
	...
	if (sys.isValid(namespace)) {
		subscriber.namespace = pageContext.namespace;
	}
}
...
UIComp.prototype.center = function() {
	var c = $(this.comp);
	var x = ($(window).width() / 2) - (c.width() / 2), y = ($(window).height() / 2)
			- (c.height() / 2);
	c.css( {
		left : x,
		top : y
...
UIDataTable.prototype.deleteEntity = function(entityId) {
	...
	if (entityId != null && confirm("Do you really want to delete?")) {
		resourceStore.deleteResourceById(this.entityName, entityId);
	}
...
UIDataTable.prototype.setSelection = function(e, row) {
	...
	variable.publish(this.comp.getAttribute("var"), value, e);
}
...
UIDataTable.prototype.load = function(filter) {
	...
	try {
		...
		resources = resourceStore.getResources(this.entityName, filter);
	} catch (e) {
		return;
	}
	...
	if (!sys.isValid(resources) || resources.length == 0) {
		...
		strBuilder
				.append("<tr height='100' valign='middle'><td align='center'><div style='line-height:100px;text-align:center'> -- No items found -- </div></td></tr>");
	} else {
		var isEven = false;
		var cellRenderer = table.attr("cellrenderer");
		for ( var i = 0, resource; resource = resources[i++];) {
			this.addRow(resource, strBuilder, isEven);
			isEven = !isEven;
...
UIDataTable.prototype.addRow = function(resource, strBuilder, isEven) {
	var table = $(this.comp)
	var headers = $('.headerRow', table)[0].childNodes;
	...
	strBuilder.append(" rid='").append(resource['id']).append("'>");
	...
	for ( var j = 0, header; header = headers[j++];) {
		...
		if (rsel == "multiple") {
			...
			var attr = header.getAttribute("attr");
			...
			if (sys.isValid(cellRenderer)) {
				...
				if (sys.isValid(attr)) {
					value = objectHelper.getAttrValue(resource, attr);
					if (!sys.isValid(value)) {

						if (el.containsVariable(attr)) {
							attr = el.getVariableName(attr);
							value = objectHelper.getAttrValue(resource, attr);
						}
					}

				} else {
					var expr = header.getAttribute("expr");
					if (sys.isValid(expr)) {
						value = el.substituteVariables(expr, resource, true);
					}
				}
		...
		for ( var i = 0, tr; tr = trs[i++];) {
			...
			tr.onclick = function(e) {
				if (sys.isValid(this.parentNode)) {
					if (selected != null) {
						selected.className = regClass;
					}

					regClass = this.className;
					if (!sys.isValid(regClass)) {
						regClass = "";
					}

					this.className = regClass + " selected";
					selected = this;
					e = eventManager.getEvent(e);
					this.parentNode.dataTable.publishSelection(e, this);
				}
			};
...
UI.prototype.init_toolbar = function(node) {
	...
	node.toolbar = new UIToolbar(node);
}
...
UIToolbar.prototype.init = function() {
	...
	var resource = this.comp.getAttribute("resource");
	...
	for ( var i = 0, b; b = buttons[i++];) {
		...
		if (button.onclick == null || button.onclick == undefined) {
			...
			button.onclick = function(e) {
				...
				case "buttonNew":
					e.name = EventName.add(resource);
					e.data = resource;
					event = e;
					
					break;
				case "buttonDelete":
					e.name = EventName.remove(resource);
					e.data = resource;
					event = e;
					break;
				case "buttonSearch":
					break;
				case "buttonSave":
					e.name = EventName.save(resource);
					e.data = resource;
					event = e;
					break;
				default :
...
WebServer.prototype = {
	...
	post : function(url, data, callback) {
		return this.invoke(url, 'DELETE', data, callback);
	},
...
EntityStore.prototype = {
	...
	getFilterPath : function(entity, filter) {
	...
	get : function(entityName, id, params, callback) {
		var result = this.server.get(this.getIdPath(entityName, id, params), callback);
	...
	mget : function(entityName, filter, callback) {
		...
		var path = this.getFilterPath(entityName, filter);

		return this.server.get(this.getFilter(entityName, filter), callback);
	...
	save : function(entityName, entity) {
		...
		return this.server.post(url, JSON.stringify(entity),
				"application/json");
	},
	...
	mremove : function(entityName, filter) {
		...
		return this.server.remove(this.getFilterPath(entityName, filter));
	}
};
...
function toolbarCommandDeployAppClicked() {
	entityStore.get("Application", "Dewbag-Retail","deploy");
}
